...
{
  const auto & member =
    *reinterpret_cast<const std::array<message_info::msg::RobotCommand, 16> *>(untyped_member);
  return &member[index];
}
...
{
  auto & member =
    *reinterpret_cast<std::array<message_info::msg::RobotCommand, 16> *>(untyped_member);
  return &member[index];
}
